	...
	struct nand_bbt_descr *bd, int chip)
{
	...
	for (i = startblock; i < numblocks;) {
		...
		else
		{
			...
			if(ret <0)
			{
				printk(KERN_ERR "create_bbt(): ret %d\n",  ret);
				printk(KERN_ERR "create_bbt(): len %d\n",  len);
				printk(KERN_ERR "create_bbt(): startblock %d\n",  startblock);
				printk(KERN_ERR "create_bbt(): numblocks %d\n",  numblocks);
				printk(KERN_ERR "create_bbt(): i %d\n",  i);
				printk(KERN_ERR "create_bbt(): chip %d\n",  chip);
				printk(KERN_ERR "create_bbt(): mtd->size %d\n",  mtd->size);
				printk(KERN_ERR "create_bbt(): this->bbt_erase_shift %d\n",  this->bbt_erase_shift);
				printk(KERN_ERR "create_bbt(): CONFIG_MTD_BADBLOCK_FLAG_PAGE %d\n",  CONFIG_MTD_BADBLOCK_FLAG_PAGE);
			}
		}
...
int nand_scan_bbt(struct mtd_info *mtd, struct nand_bbt_descr *bd)
{
	...
	if (!td) {
		if ((res = nand_memory_bbt(mtd, bd))) {
			printk(KERN_ERR "nand_bbt: Can't scan flash and build the RAM-based BBT,%d\n",res);
			kfree(this->bbt);
			this->bbt = NULL;
		}
		return res;
